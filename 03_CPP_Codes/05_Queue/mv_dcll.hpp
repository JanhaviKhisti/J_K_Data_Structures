...
public:
	...
	{
		// Code
		return( GenericInsert(mp_tail, new Node<T_LIST>( new T_LIST(new_data) ), mp_tail->mp_next) );
	}*/
	...
	T_LIST Tail() const { return(mp_tail->m_data); }
	...
	//	O(1)
	ret_t Swap(MV_DCLL<T_LIST>* plist)
	{
		// Code
		Node<T_LIST>*	list_sentinel = plist->mp_head;
		size_t	list_size = plist->m_size;

		//	Give Your list to plist
		plist->mp_head = this->mp_head;
		plist->m_size = this->m_size;
		plist->mp_tail = plist->mp_head->mp_prev;

		//	Take the plist's list in yourself
		this->mp_head = list_sentinel;
		this->m_size = list_size;
		this->mp_tail = this->mp_head->mp_prev;
